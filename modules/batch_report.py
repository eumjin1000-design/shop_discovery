...
def write_batch_report(rows: list[dict], out_dir: str = "output") -> str:
    ...
    _header(ws, "순위", "카테고리(EN)", "카테고리(한글)", "총점 /100",
            "판정", *factor_names, "Verdict")

    for rank, r in enumerate(rows, start=1):
        excel_row = rank + 1
        ws.cell(row=excel_row, column=1, value=rank)
        ws.cell(row=excel_row, column=2, value=r["name"])
        ws.cell(row=excel_row, column=3, value=ko_translate(r["name"]))
        ws.cell(row=excel_row, column=4, value=round(r["total"], 1))
        cell = ws.cell(row=excel_row, column=5, value=r["decision"])
        cell.font = Font(bold=True)
        if r["decision"] in _DECISION_FILL:
            cell.fill = _DECISION_FILL[r["decision"]]
        for j, (_bn, score, _mx) in enumerate(r["breakdown"]):
            ws.cell(row=excel_row, column=6 + j, value=round(score, 1))
        sc = ws.cell(row=excel_row, column=6 + len(factor_names),
                     value=r.get("summary", ""))
    ...
    for i in range(len(factor_names)):
        ws.column_dimensions[chr(ord("F") + i)].width = 13
    ws.column_dimensions[chr(ord("F") + len(factor_names))].width = 80
    ws.freeze_panes = "A2"
